...
We now train a Neural Network with the data given.

For Neural Networks, we need to normalize the data (to ensure convergence). Min-max scaling is used.
```{r}
maxs <- apply(predict_set, 2, max) 
mins <- apply(predict_set, 2, min)
scaled <- as.data.frame(scale(predict_set, center = mins, scale = maxs - mins))
summary(scaled)
```
Apply Scaling to train and test sets
```{r}
predict_set.scaled_train <- scaled[train_ind,]
predict_set.scaled_test <- scaled[-train_ind,]

nrow(predict_set.scaled_train)
nrow(predict_set.scaled_test)
```
```{r}
head(df_year)
start = min(df_year$year)
end = max(df_year$year)

```


```{r}
#df_artist[df$artists in ]
dplyr::filter(df_genre, grepl('pop', genres))

```


```{r}
summary(predict_set.train$year)
start = min(predict_set.train$year)
end = max(predict_set.train$year)
start
end
predict_set.ts = ts(predict_set.train$popularity, start = start, end = end, frequency = 1)
```
```{r}
summary(predict_set.test$year)
start = min(predict_set.test$year)
end = max(predict_set.test$year)
start
end
predict_set.ts_test = ts(predict_set.test$popularity, start = start, end = end, frequency = 1)
```


```{r}
predict_set.nn <- mlp(y = predict_set.ts)
predict_set.nn
```
```{r}
plot(predict_set.nn)
...
```{r}
predict_set.ts_test_forecast = forecast(predict_set.nn, h=12)
plot(predict_set.ts_test_forecast)

```

```{r}
plot(predict_set.train$popularity)
```


```{r}
maxs <- apply(predict_set.ts, 2, max) 
mins <- apply(predict_set.ts, 2, min)
scaled <- as.data.frame(scale(predict_set.ts, center = mins, scale = maxs - mins))
...
```{r}
predict_set.nn <- elm(predict_set.scaled_train)
...
n <- names(predict_set.scaled_train)
f <- as.formula(paste("popularity ~", paste(n[!n %in% "popularity"], collapse = " + ")))
#predict_set.nn_train <- elm(predict_set.scaled_train)
predict_set.nn_train <- neuralnet(f,data=predict_set.scaled_train, hidden = c(4,2),linear.output=T,act.fct="tanh",lifesign ="full", threshold=0.1)
plot(nn_train)
...
```{r}
## 80% as train_set
tmp_size <- floor(0.10 * nrow(predict_set))

set.seed(123)
train_ind <- sample(seq_len(nrow(predict_set)), size = tmp_size)

tmp_train <- predict_set[train_ind, ]
...
rf_tmp = randomForest(popularity ~ acousticness + danceability + duration_ms+ energy+ explicit+ instrumentalness+ liveness+ loudness+ mode+ speechiness+ tempo+ valence+ year+ key_1+ key_2+ key_3+ key_4+ key_5+ key_6+ key_7+ key_8+ key_9+ key_10+ key_11, data = tmp_train)
...
predictions = predict(rf_tmp, newdata = tmp_train)
predictions_test = predict(rf_tmp, newdata = predict_set.test)


rmse <- rmse(tmp_train$popularity, predictions)
rmse

rmse <- rmse(tmp_train$popularity, predictions_test)
rmse
```
...
model<-train(popularity~., data=predict_set.train, method='rf') 
